    ...
    for (; n < numel; n += LAST_DIM_UNROLL_FACTOR) {
        ...
        if (items_left_in_last_dim >= LAST_DIM_UNROLL_FACTOR) {
            ...
            out_data[out_index + 0 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 0 * a_ldim_stride], b_data[b_index + 0 * b_ldim_stride]);
            out_data[out_index + 1 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 1 * a_ldim_stride], b_data[b_index + 1 * b_ldim_stride]);
            out_data[out_index + 2 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 2 * a_ldim_stride], b_data[b_index + 2 * b_ldim_stride]);
            out_data[out_index + 3 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 3 * a_ldim_stride], b_data[b_index + 3 * b_ldim_stride]);
            out_data[out_index + 4 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 4 * a_ldim_stride], b_data[b_index + 4 * b_ldim_stride]);
            out_data[out_index + 5 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 5 * a_ldim_stride], b_data[b_index + 5 * b_ldim_stride]);
            out_data[out_index + 6 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 6 * a_ldim_stride], b_data[b_index + 6 * b_ldim_stride]);
            out_data[out_index + 7 * out_ldim_stride] =
                    Op::template call<Out, A, B>(a_data[a_index + 7 * a_ldim_stride], b_data[b_index + 7 * b_ldim_stride]);
            // }
        } else {
            unroll_size = items_left_in_last_dim;
            for (int j = 0; j < items_left_in_last_dim; ++j) {
                out_data[out_index + j * out_ldim_stride] =
                        Op::template call<Out, A, B>(a_data[a_index + j * a_ldim_stride], b_data[b_index + j * b_ldim_stride]);
            }
        }
