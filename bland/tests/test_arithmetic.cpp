...
TEST_CASE("add", "[ops][arithmetic]") {
    SECTION("addition", "addition ops") {
        {
            ...
            REQUIRE_THAT(std::vector<float>(x.data_ptr<float>(), x.data_ptr<float>() + x.numel()),
                         BlandWithinAbs(
                            std::vector<float>(test_array.data_ptr<float>(), test_array.data_ptr<float>() + test_array.numel()),
                            0.0001f));

            auto expected =
                    bland::arange(10.0f, 60.0f, 1.0f, DLDataType{.code = DLDataTypeCode::kDLFloat, .bits = 32});
            REQUIRE_THAT(std::vector<float>(result.data_ptr<float>(), result.data_ptr<float>() + result.numel()),
                         BlandWithinAbs(
                            std::vector<float>(expected.data_ptr<float>(), expected.data_ptr<float>() + expected.numel()),
                            0.0001f));
        }
    }
    SECTION("division", "division ops") {
        { // 2D divide by scalar
            ...
            auto expected =
                    bland::arange(0.0f, 5.0f, 0.1f, DLDataType{.code = DLDataTypeCode::kDLFloat, .bits = 32});
            REQUIRE_THAT(std::vector<float>(result.data_ptr<float>(), result.data_ptr<float>() + result.numel()),
                         BlandWithinAbs(
                            std::vector<float>(expected.data_ptr<float>(), expected.data_ptr<float>() + expected.numel()),
                            0.0001f));
            ...
            REQUIRE_THAT(std::vector<float>(x.data_ptr<float>(), x.data_ptr<float>() + x.numel()),
                         BlandWithinAbs(
                            std::vector<float>{
                                ...
                                40, 41, 4.2, 43, 44, 45, 46, 47, 48, 49,
                            }, .0001f));
        }
        { // Slice both dims
            auto test_array =
            ...
            REQUIRE_THAT(std::vector<float>(x.data_ptr<float>(), x.data_ptr<float>() + x.numel()),
                         BlandWithinAbs(
                            std::vector<float>{
                                ...
                                20, 2.1, 2.2, 2.3, 24, 25, 26, 27, 28, 29,
