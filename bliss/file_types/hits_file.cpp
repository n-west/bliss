...
void bliss::write_hits_to_file(std::vector<hit> hits, std::string_view file_path) {
    ...
    for (size_t hit_index=0; hit_index < hits.size(); ++hit_index) {
        ...
        signal_builder.setDriftSteps(this_hit.time_span_steps); // redundant with numTimesteps but was a rounded-up power of 2 in seticore
...
std::vector<hit> bliss::read_hits_from_file(std::string_view file_path) {
    ...
    while (true) {
        try {
            ...
            this_hit.time_span_steps = signal_reader.getDriftSteps();
...
void bliss::write_scan_hits_to_file(scan scan_with_hits, std::string_view file_path) {
    ...
    for (size_t hit_index=0; hit_index < number_hits; ++hit_index) {
        ...
        this_signal.setFrequency(this_hit.start_freq_MHz);
        this_signal.setIndex(this_hit.start_freq_index);
        this_signal.setDriftSteps(this_hit.time_span_steps); // redundant with numTimesteps but was a rounded-up power of 2 in seticore
        this_signal.setDriftRate(this_hit.drift_rate_Hz_per_sec);
        this_signal.setSnr(this_hit.snr);
        // this_signal.setCoarseChannel(); // not yet looking at multiple coarse channels
        // this_signal.setBeam(); // not yet looking at multiple beams
        this_signal.setNumTimesteps(this_hit.time_span_steps); // for us, redundant with driftSteps
        // the power and incoherentpower are redundant in bliss, but were not in seticore
        // the seticore power was normalized for computing an SNR, the incoherentpower was just the integrated power
        this_signal.setPower(this_hit.power);
        this_signal.setIncoherentPower(this_hit.power);
    ...
    auto in_file = detail::raii_file_for_read(file_path);
        ...
        try {
            ...
            for (size_t hit_index=0; hit_index < number_hits; ++hit_index) {
                ...
                hit this_hit;
                this_hit.start_freq_MHz = this_detection.getFrequency();
                this_hit.start_freq_index = this_detection.getIndex();
                this_hit.time_span_steps = this_detection.getDriftSteps();
                this_hit.drift_rate_Hz_per_sec = this_detection.getDriftRate();
                this_hit.snr = this_detection.getSnr();
                this_hit.time_span_steps = this_detection.getNumTimesteps();
                this_hit.power = this_detection.getPower();
...
void bliss::write_observation_target_hits_to_files(observation_target observation_with_hits, std::string_view base_filename) {
